...
// ButteraugliComparatorの名前空間でのButteraugliComparatorらしいが、
// これは関数? おそらく入力画像と最終的な出力画像を比較するクラスのはず..

// http://vivi.dyndns.org/tech/cpp/class-basic.html
// コンストラクタの実装っぽい
// コンストラクタは、上記の様にクラス宣言の外で、 型を持たない関数の形式（クラス名::クラス名() { ... }）で実装する。
// privateのメンバ変数の初期化もしている
...
// 実際に比較するメソッドだとおもうが、読めない。
void ButteraugliComparator::Compare(const OutputImage& img) {
  std::vector<ImageF> rgb0 =
      ::butteraugli::OpsinDynamicsImage(LinearRgb(width_, height_, rgb_orig_)); // butteraugliのクラス
...
// 匿名名前空間
// mainとかの名前空間を汚さないために利用する
// http://kaworu.jpn.org/cpp/%E7%84%A1%E5%90%8D%E5%90%8D%E5%89%8D%E7%A9%BA%E9%96%93
namespace {
