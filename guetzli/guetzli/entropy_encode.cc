...
// 各々0~13(出現する最後まで)を考えると,
// index_right_or_value_は順番にiでforの値が入っているので,
// 対応するlevelを入れる
bool SetDepth(int p0, HuffmanTree *pool, uint8_t *depth, int max_depth) {
  ...
  while (true) {
    if (pool[p].index_left_ >= 0) {
      ...
      // 対応する葉のindex(元hisotgrams)のdepthに打ち込む
...
void CreateHuffmanTree(const uint32_t *data,
  ...
  for (uint32_t count_limit = 1; ; count_limit *= 2) {
    ...
    // 一番出現回数が小さいものからひとつのセットにしていき,
    // 小さい出現回数の合計を作っていく。 => ハフマンtreeの構築
