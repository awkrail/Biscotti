...
bool ReadPNG(const std::string& data, int* xsize, int* ysize,
  ...
  *ysize = png_get_image_height(png_ptr, info_ptr);
  ...
  switch (components) {
    ...
    case 3: {
      // RGB
      // 配列にrgbデータからとってきたものをrgbにコピーしている
      // memcpyの方が早いのかな
      // ここから読む
      for (int y = 0; y < *ysize; ++y) {
        const uint8_t* row_in = row_pointers[y];
...
std::string ReadFileOrDie(const char* filename) {
  ...
  // C++でファイルの大きさを取り出すことができるテクニック。
  // fseekを利用してファイルの最後までファイルポインタを移動させて
  // ftellを利用して場所を取り出す。 => ファイルの大きさになる
  ...
  std::unique_ptr<char[]> buf(new char[buffer_size]); // 領域を確保する
  while (!feof(f)) {
    // ファイルの先頭からbuffer_sizeぶんをすすめる
...
int main(int argc, char** argv) {
  ...
  //argvに引数データ, argv[opt_idx]に画像データが入る
  //stringの文字列にする
  ...
  // static castっていうのは<>の中の型に自動的に型変換するらしいよ
  // params.butteraugli_targetは, quality.h, quality.ccに詳しく書かれてて,
  // qualityの引数に応じて変わるっぽい
  params.butteraugli_target = static_cast<float>(
      guetzli::ButteraugliScoreForQuality(quality));
  std::cout << "butteraugli score: " << params.butteraugli_target << std::endl;

  std::string raw_filename = std::string(argv[opt_idx]);
  params.filename = std::regex_replace(raw_filename.substr(5), std::regex("(\.jpg|\.jpeg|\.png)"), "");
  ...
  // guetzli::Processメソッドを使うことで, out_dataのメモリに直接書き込んでるはず
  if (in_data.size() >= 8 &&
    ...
    // ここからguetzli::Processが実行され, これによりout_dataにデータが書き込まれる。
