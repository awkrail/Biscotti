...
void InitJPEGDataForYUV444(int w, int h, JPEGData* jpg) {
  ...
  jpg->MCU_rows = (h + 7) >> 3; //なぜ右シフトするのかはわからんが、8*8のブロックの数だろう
  jpg->MCU_cols = (w + 7) >> 3;
  jpg->quant.resize(3); // jpgのquant/ YUVのみっつで3?
  jpg->components.resize(3);

  // jpgのそれぞれのコンポーネントに属性を追加
  // YCbCrのそれぞれのDCTテーブルの係数が入っている?
  for (int i = 0; i < 3; ++i) {
    ...
    c->coeffs.resize(c->num_blocks * kDCTBlockSize); // coeffs -> DCTテーブルの係数
  }
}
...
void SaveQuantTables(const int q[3][kDCTBlockSize], JPEGData* jpg) {
  ...
  for (size_t i = 0; i < jpg->components.size(); ++i) {
    ...
    // copyするだけ
    // すでにもってることが多いので{1,1,...1}の量子化テーブルのデータが入る。
    bool found = false;
