...
struct JPEGData {
  ...
  int width; // 画像の幅
  int height; // 画像の高さ
  int version; // jpegにはバージョンが二つあるらしい
  // jpegの画像は, YCbCrで保存され, CbCrはサンプリングして容量を減らす?
  int max_h_samp_factor; // 横方向のサンプリング
  int max_v_samp_factor; // 縦方向のサンプリング
  int MCU_rows; // MCU単位の行数
  int MCU_cols; // MCU単位の列数
  int restart_interval; // http://hp.vector.co.jp/authors/VA032610/JPEGFormat/marker/DRI.htm
  std::vector<std::string> app_data; // 最初のデータ?
  std::vector<std::string> com_data; // 圧縮後?
  std::vector<JPEGQuantTable> quant; // 量子化テーブル?(8*8)
  std::vector<JPEGHuffmanCode> huffman_code; // ハフマン符号化?
  std::vector<JPEGComponent> components; // jpegのコンポーネントらしいが必要性は不明
  std::vector<JPEGScanInfo> scan_info; // ハフマン符号絡み
  std::vector<uint8_t> marker_order; // わからない
  std::vector<std::string> inter_marker_data; // わからない
  std::string tail_data; // わからない
  const uint8_t* original_jpg; // わからない
  size_t original_jpg_size; // umm..
  JPEGReadError error; // errorかどうかを見る列挙型
};
