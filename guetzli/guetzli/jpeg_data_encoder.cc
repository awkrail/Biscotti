...
// inline関数によって処理を高速化する(呼び出しの時間を削る)
...
bool EncodeRGBToJpeg(const std::vector<uint8_t>& rgb, int w, int h,
  ...
  InitJPEGDataForYUV444(w, h, jpg); // YUVとして初期化。 DCTテーブルの追加
  AddApp0Data(jpg); // ヘッダーにApp0を付加する
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < kDCTBlockSize; ++j) {
      int v = quant[idx]; // 定数=>128行目に配列があるっぽい
      jpg->quant[i].values[j] = v;
      iquant[idx++] = ((1 << kIQuantBits) + 1) / v; // (1 << 16) + 1 / 1?
    }
  }

  //↑の操作でjpg->quantの配列はすべて1で初期化
  //iquantはすべて65537
  ...
  for (int block_y = 0; block_y < jpg->MCU_rows; ++block_y) {
    for (int block_x = 0; block_x < jpg->MCU_cols; ++block_x) {
      coeff_t block[3 * kDCTBlockSize]; // coeff_t -> int16_t
      // RGB->YUV transform.
      // 各ブロックごとに, blockにYUVのデータが入る。
      // 下の二重ループで画像のデータを走査してブロックごとにYUV変換する。
      for (int iy = 0; iy < 8; ++iy) {
        for (int ix = 0; ix < 8; ++ix) {
          int y = std::min(h - 1, 8 * block_y + iy);
          int x = std::min(w - 1, 8 * block_x + ix);
          int p = y * w + x;
          // &block[0] と blockで渡すことに違いはあるの?
          RGBToYUV16(&rgb[3 * p], &block[8 * iy + ix]); // pointerで渡す
        }
      }
      // DCT
      // blockごとにDCT変換の計算をする。
