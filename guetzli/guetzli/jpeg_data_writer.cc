...
bool EncodeSOF(const JPEGData& jpg, JPEGOutput out) {
  ...
  data[pos++] = 0xff;
  data[pos++] = 0xc1; // sof1
  data[pos++] = static_cast<uint8_t>(marker_len >> 8); // データの長さ
  data[pos++] = marker_len & 0xff; // データの長さ
  data[pos++] = kJpegPrecision; // 精度
  data[pos++] = jpg.height >> 8;
  data[pos++] = jpg.height & 0xff; // 0xffは1になっている数字を数える?
  data[pos++] = jpg.width >> 8;
  data[pos++] = jpg.width & 0xff;
  data[pos++] = static_cast<uint8_t>(ncomps);
  for (size_t i = 0; i < ncomps; ++i) {
    data[pos++] = jpg.components[i].id; // Y => 0, Cb => 1, Cr => 2
    data[pos++] = ((jpg.components[i].h_samp_factor << 4) |
                      (jpg.components[i].v_samp_factor)); // サンプリング数
    const size_t quant_idx = jpg.components[i].quant_idx;
    if (quant_idx >= jpg.quant.size()) {
      return false;
    }
    data[pos++] = jpg.quant[quant_idx].index;
    // guetzliではquant_idxは一つしかない
    // なんで, 出力される量子化テーブルのidは一つしかない。
  }
  return JPEGWrite(out, &data[0], pos);
}
...
void BuildHuffmanCodeTable(const int* counts, const int* values,
  ...
  int huffsize[256];
  int p = 0;
  for (int l = 1; l <= kJpegHuffmanMaxBitLength; ++l) {  // kJpegHuffmanMaxBitLength=16
    int i = counts[l];
    while (i--) huffsize[p++] = l; // i--はiが0になったらfalseを返す
  }
...
// YCbCrのそれぞれ(のDCTテーブルを1で量子化したもの)に対して,
// DC成分の差分を記憶し, ヒストグラムにしたもの。
void BuildDCHistograms(const JPEGData& jpg, JpegHistogram* histo) {
  for (size_t i = 0; i < jpg.components.size(); ++i) {
    const JPEGComponent& c = jpg.components[i];
    JpegHistogram* dc_histogram = &histo[i]; // YCbCrのそれぞれではないか
      ...
      for (int mcu_x = 0; mcu_x < jpg.MCU_cols; ++mcu_x) {
        for (int iy = 0; iy < c.v_samp_factor; ++iy) {
          for (int ix = 0; ix < c.h_samp_factor; ++ix) {
            ...
            int nbits = Log2Floor(diff) + 1; // diffのある部分を記憶する
            dc_histogram->Add(nbits); // diffのあるところを記録する
  ...
  while (*num > 1) {
    ...
    if (cost_combined < costs[last] + costs[second_last]) {
      ...
      memcpy(&depth[second_last * JpegHistogram::kSize], depth_combined,
             sizeof(depth_combined)); // combined(YCbCrをまぜたやつ)での結果をdepthにコピー
...
bool BuildAndEncodeHuffmanCodes(const JPEGData& jpg, JPEGOutput out,
  ...
  BuildDCHistograms(jpg, &histograms[0]); // YCbCrのそれぞれの量子化テーブルのdiffのヒストグラム

  // Cluster DC histograms.
  size_t num_dc_histo = ncomps;
  int dc_histo_indexes[kMaxComponents]; // 4
  std::vector<uint8_t> depths(ncomps * JpegHistogram::kSize); // 各フィルタごとのハフマン符号について
  // ハフマンtreeをYCbCrのDC成分のみで作る感じ
  ClusterHistograms(&histograms[0], &num_dc_histo, dc_histo_indexes,
                    &depths[0]); // 統合したヒストグラムになってる
  // histograms => 多分ここにハフマンtreeの結果(出現回数)
  // dc_histo_indexes => YCbCrのどれがどのhistogramsに対応しているか?
  // depths => 一つに統合したから減っているんだろうけどなんのdepthかよくわからない
  ...
  // ここまでで一番コストが小さくなるようにDC, AC成分のハフマン符号化ができるようになった。
  ...
  const size_t sos_marker_len = 6 + 2 * ncomps; // 6(固定ぶん) + 2(成分ID+ハフマンTable)
  std::vector<uint8_t> data(dht_marker_len + sos_marker_len + 4);
  size_t pos = 0;

  // DHT(ハフマン符号化テーブル)のスタート
  data[pos++] = 0xff;
  data[pos++] = 0xc4;
  ...
  for (int i = 0; i < num_histo; ++i) {
    ...
    // depthはindexがhistoに対応し, データがハフマン符号での深さ
    // countsはビットの長さi数必要なものを数えそう。
    // valuesはiビットの符号数の関連する値が入りそう => 実際に割り当てられるbitっぽい
    BuildHuffmanCode(&depths[i * JpegHistogram::kSize], counts, values); // この関数でそれをする
    HuffmanCodeTable table;
