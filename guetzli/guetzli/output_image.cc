...
void OutputImageComponent::SetCoeffBlock(int block_x, int block_y,
  ...
  assert(block_y < height_in_blocks_);
  int offset = (block_y * width_in_blocks_ + block_x) * kDCTBlockSize;
  memcpy(&coeffs_[offset], block, kDCTBlockSize * sizeof(coeffs_[0])); // Blockのこぴー
  uint8_t idct[kDCTBlockSize];

  // 逆量子化されたデータが入る
  // ComputeBlockIDCT -> 与えられたDCT係数を逆量子化
  // UpdatePixelsForBlock -> IDCTの結果からピクセルを計算
...
// JPEGData型にあるDCT係数でのデータを逆量子化を行った上で変更する
void OutputImageComponent::CopyFromJpegComponent(const JPEGComponent& comp,
  ...
  for (int block_y = 0; block_y < height_in_blocks_; ++block_y) {
    ...
    for (int block_x = 0; block_x < width_in_blocks_; ++block_x) {
      ...
      SetCoeffBlock(block_x, block_y, block); //この関数がpixels_とcoeffから色々やるので注意
...
// Dump_Csv (my function)
void OutputImageComponent::DumpDctCoeffToCsv(const std::string filename){
    // DCT係数の取得 -> Csvfileへダンプする
    coeff_t coeff_v[num_blocks_][kDCTBlockSize];
    for(int block_y = 0; block_y < height_in_blocks_; ++block_y){
        for(int block_x = 0; block_x < width_in_blocks_; ++block_x){
            coeff_t block[kDCTBlockSize];
            int idx = width_in_blocks_ * block_y + block_x; 

            // Get DCT Coeffs
            GetCoeffBlock(block_x, block_y, block);
            memcpy(&coeff_v[idx][0], &block[0], kDCTBlockSize * sizeof(block[0])); 
        }
    }

    // CSVへダンプする
    std::string fn = filename + ".csv";
    std::ofstream ofs(fn, std::ios::app);
    for(int i = 0; i < num_blocks_; i++){
        for(int j = 0; j < kDCTBlockSize; j++){
            if(j != kDCTBlockSize-1) ofs << coeff_v[i][j] << ",";
            else ofs << coeff_v[i][j];
        }
        ofs << std::endl;
    }
}
...
// グローバル最適化(量子化テーブル)
// DCT係数の取得->QuantizeBlockで新たな量子化テーブルの適用->SetCoeffBlockで変更
...
void OutputImage::DumpDctCoeffToCsv(const std::string filename){
    for(int c=0; c<3; ++c){
        std::cout << "==== channel:" << c << "=====" << std::endl;
        components_[c].DumpDctCoeffToCsv(filename);
    }
}
