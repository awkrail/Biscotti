...
// なんでここに無名名前空間つくったんだろうか
// 同一ファイル内からは見えるけど， 他のファイルからは見えないグローバル変数などをつくることができる
// http://exlight.net/devel/cpp/unnamed_namespace.html
namespace {

static const size_t kBlockSize = 3 * kDCTBlockSize; // 3*64 = 192
...
// q_inのデータを消し去る
// c.coeffsのDCT係数を逆量子化する
// jpgのデータに量子化テーブルを保存(全部1)
void RemoveOriginalQuantization(JPEGData* jpg, int q_in[3][kDCTBlockSize]) {
  for (int i = 0; i < 3; ++i) {
    JPEGComponent& c = jpg->components[i];
    const int* q = &jpg->quant[c.quant_idx].values[0]; // 量子化テーブルのpointer
    memcpy(&q_in[i][0], q, kDCTBlockSize * sizeof(q[0])); // 量子化テーブルのコピー
    for (size_t j = 0; j < c.coeffs.size(); ++j) {
      c.coeffs[j] *= q[j % kDCTBlockSize]; // c.coeffs->逆量子化
    }
  }
  int q[3][kDCTBlockSize];
  for (int i = 0; i < 3; ++i)
...
// どういう動きするのかいまいちよくわからない..
// このコールバック関数が書き込む処理をしているのでは?
// うまくいったかどうかをboolで確認する感じ?
// データ分の長さで書き込めていたらtrueが帰ってくる
...
QuantData Processor::TryQuantMatrix(const JPEGData& jpg_in,
                                    ...
                                    OutputImage* img) {
  QuantData data;
  // QuantDataは, q(量子化テーブル)と,
  // jpg_size, dist_okからなる
  
  // 画像データから,encoded_jpgを作成する
  // 量子化テーブルの作成
  ...
  std::string encoded_jpg;
  {
    ...
    img->SaveToJpegData(&jpg_out); //最適化processした後のimageがくる感じ
    OutputJpeg(jpg_out, &encoded_jpg); // 一回encoded_jpgに入れる
  }
...
bool Processor::SelectQuantMatrix(const JPEGData& jpg_in, const bool downsample,
  ...
  for (;;) {
    ...
    qgen.Add(data);
    if (CompareQuantData(data, best)) { // 前よりも小さくなることを確認する
      best = data;
      if (data.dist_ok && !comparator_->DistanceOK(target_mul_low)) {
        break; // targetまで十分にたどり着けば終わり
      }
    }
  }
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    for (int k = 1; k < kDCTBlockSize; ++k) {
      ...
      if (block[idx] != 0) {
        ...
        // 左上はほぼ100%入らないようになっているっぽい
        // 0のものはもう興味がないようになっている
...
void Processor::SelectFrequencyMasking(const JPEGData& jpg, OutputImage* img,
  ...
  std::cout << "image file width: " << width << std::endl;
  std::cout << "image file height: " << height << std::endl;
  std::cout << "ncomp: " << ncomp << std::endl;
  std::cout << "last_c: " << last_c << std::endl;
  std::cout << "factor_x: " << factor_x << std::endl;
  std::cout << "factor_y: " << factor_y << std::endl;
  ...
  for (int block_y = 0, block_ix = 0; block_y < block_height; ++block_y) {
    for (int block_x = 0; block_x < block_width; ++block_x, ++block_ix) {
      // kBlockSizeはブロック(YCbCr)の3フィルタ分で3*64=192
      ...
      for (int c = 0; c < 3; ++c) {
        if (comp_mask & (1 << c)) {
          ...
          img->component(c).GetCoeffBlock(block_x, block_y,
                                          &block[c * kDCTBlockSize]); // これどういう書き方なんだろう
      ...
      // よく見てみると, compに入っている値とblockに入っている値とで異なっている
      block_order.clear();

      // 全MCUでzeroになる部分を計算する
      // ブロックごとに(MCUごとに)計算を行っているはず
      // target_distanceよりerrが小さくないとblock_order(各ピクセルの係数のところにerrが入ったもの)はzeroにはならないらしい
  ...
  for (int direction : {1, -1}) {
    for (;;) {
      ...
      comparator_->Compare(*img);
      MaybeOutput(encoded_jpg); // ここで最適化の結果を出力している
...
bool Processor::ProcessJpegData(const Params& params, const JPEGData& jpg_in,
  ...
  // butteraugliのqualityでbutteraugli_targetの値はきまるっぽい
  // 84を下回れば使えなくなります。
  ...
  // encoded_jpgに結構いろいろしてる感じな気もする => ビンゴ
  ...
  // 元のデータと,比較するために初期化
  {
  JPEGData jpg = jpg_in; // データのコピー
  RemoveOriginalQuantization(&jpg, q_in);
  OutputImage img(jpg.width, jpg.height);
  img.CopyFromJpegData(jpg); // OutputImage
  comparator_->Compare(img);
  }
  ...
  // 420にダウンサプリングする
  // 少なくとも一回は動く
  std::cout << "try_420: " << try_420 << std::endl;
  std::cout << "force_420: " << force_420 << std::endl;
  ...
  for (int downsample = force_420; downsample <= try_420; ++downsample) {
    ...
    int best_q[3][kDCTBlockSize];
    memcpy(best_q, q_in, sizeof(best_q)); // best_qにq_inを入れる
    ...
    // JpegData jpg -> OutputImage img -> best_q(量子化テーブル適用) -> MaybeOutputでencoded_jpgにコピー
    img.CopyFromJpegData(jpg);
    img.ApplyGlobalQuantization(best_q);

    std::string filename = "csv/data/" + params.filename;
    img.DumpDctCoeffToCsv(filename);

    std::cout << 3 << std::endl;

    
    // 実際にMaybeOutputでencoded_jpegにデータが入るから下の処理を消しても動く

    // SelectFrequencyMaskingが死ぬほど遅い.
    if (!downsample) {
    ...
    std::string after_g_filename = "csv/after_data/" + params.filename;
    img.DumpDctCoeffToCsv(after_g_filename);
  }
...
//二つの同じ名前の関数があるが,
//C++って引数の数で関数名を区別できたっけ　=> CではできないがC++ではできるようになったらしい。
//関数のオーバーライド: https://www.s-cradle.com/developer/sophiaframework/tutorial/Cpp/overload.html
//画像によってどっちが読み込まれるのかは変わるらしい
...
bool Process(const Params& params, ProcessStats* stats,
  ...
  // std::unique_ptrは動的にポインタを格納するものらしい
...
bool Process(const Params& params, ProcessStats* stats,
             ...
             std::string* jpg_out) {
  // JPEGData型に関しては読んだ。プロパティーの量がやばい
  JPEGData jpg;

  // よく見たら, Encode中にDCTテーブルをなんかしてないか
  ...
  // std::unique_ptrはgetというメソッドを持ち,
  // comparatorのメモリから値を渡す。
  // outのdoubleは謎の初期化が入っている
  bool ok = ProcessJpegData(params, jpg, comparator.get(), &out, stats);
  *jpg_out = out.jpeg_data;
  return ok;
}
