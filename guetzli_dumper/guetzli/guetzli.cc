...
int main(int argc, char** argv) {
  ...
  std::string out_data;
  std::string q_out_data; // 量子化テーブルのみ最適
  ...
  stats.filename = static_cast<std::string>(argv[opt_idx]);
  ...
  if (in_data.size() >= 8 &&
    ...
    if (!guetzli::Process(params, &stats, rgb, xsize, ysize, &out_data, &q_out_data)) {
    ...
    if (!guetzli::Process(params, &stats, in_data, &out_data, &q_out_data)) { 
      fprintf(stderr, "Guetzli processing failed\n");
      return 1;
    }
  }
  ...
  WriteFileOrDie(argv[opt_idx + 2], q_out_data);
  return 0;
}
