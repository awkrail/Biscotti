...
class Processor {
 public:
  bool ProcessJpegData(const Params& params, const JPEGData& jpg_in,
                       ...
                       GuetzliOutput* q_out, ProcessStats* stats);
 ...
 private:
  ...
  void DumpCoeffBlockToCsvFile(const JPEGData jpg, OutputImage* img, std::string filename);
...
// 追加した関数 Coeffを取得してCSVFileにダンプする
void Processor::DumpCoeffBlockToCsvFile(const JPEGData jpg, OutputImage* img, std::string filename){
  const int width = img->width();
  const int height = img->height();
  const int ncomp = jpg.components.size();
  std::string rm_ex = std::regex_replace(filename, std::regex("(\.jpg|\.png|\.jpeg)"), "");
  std::string f_name = std::regex_replace(rm_ex, std::regex("(.*\/)"), "");
  std::ofstream ofs;
  ofs.open("csv/" + f_name + ".csv");
  // Y -> Cb -> Crの順番に格納する
  for(int c=0; c<ncomp; ++c){
    int q_t_c[kDCTBlockSize] = { 0 };
    memcpy(q_t_c, img->component(c).quant(), sizeof(img->component(c).quant()[0])*kDCTBlockSize);
    ...
    for(int block_y=0; block_y < block_height; ++block_y){
      for(int block_x=0; block_x < block_width; ++block_x){
        ...
        img->component(c).GetCoeffBlock(block_x, block_y, &block[0]);
        for(int i=0; i<kDCTBlockSize; i++) {          
          if(i != kDCTBlockSize-1) {
            coeff_t coeff = block[i];
            if(coeff != 0){
              ofs << 1 << ",";
            }else{
              ofs << 0 << ",";
...
bool Processor::ProcessJpegData(const Params& params, const JPEGData& jpg_in,
  ...
  for (int downsample = force_420; downsample <= try_420; ++downsample) {
    ...
    q_out->jpeg_data = final_output_->jpeg_data; // 量子化テーブルのみを最適化した状態

    if (!downsample) {
      SelectFrequencyMasking(jpg, &img, 7, 1.0, false);
      // ここに最後まで最適化した状態の画像のDCT係数をCSVでdumpする関数を追加する。
      DumpCoeffBlockToCsvFile(jpg, &img, stats_->filename);
    } else {
...
bool ProcessJpegData(const Params& params, const JPEGData& jpg_in,
                     ...
                     GuetzliOutput* q_out, ProcessStats* stats) {
  Processor processor;
  return processor.ProcessJpegData(params, jpg_in, comparator, out, q_out, stats);
}

bool Process(const Params& params, ProcessStats* stats,
             const std::string& data,
             std::string* jpg_out, std::string* q_jpg_out) {
  JPEGData jpg;
  ...
  bool ok = ProcessJpegData(params, jpg, comparator.get(), &out, &q_out, stats);
  *jpg_out = out.jpeg_data;
  *q_jpg_out = q_out.jpeg_data;
  return ok;
}
...
bool Process(const Params& params, ProcessStats* stats,
  ...
  bool ok = ProcessJpegData(params, jpg, comparator.get(), &out, &q_out, stats);
  *jpg_out = out.jpeg_data; 
  *q_jpg_out = q_out.jpeg_data; // 量子化テーブルのみ最適化した時のデータ 
  return ok;
}
